...
def obs_process(obs):
    ...
    for i in range(num_env):
        ...
        for agent_type in ["resource","base","barrack","worker","light","heavy","ranged"]:
        ...
        for j in range(w):
            for k in range(h):
                if obs[i][j][k][13]!=1:
                    ...
                    if inf[14]==1:
                        agent_infos["my_info"]["resource"].append(inf_all)
                        agent_infos["opp_info"]["resource"].append(inf_all)
                    if inf[15]==1:
                        if inf[11] == 1:
...
if __name__ == "__main__":
    ...
    individual_obs = obs_process(envs.reset())

    next_done = torch.zeros(args.num_envs).to(device)

    org_next_obs = envs.reset()
    ...
    for update in range(starting_update, args.num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            individual_obs = obs_process(org_next_obs)
            ...
            try:
                if step%10!=0:
                    ...
                    next_obs, rs, ds, infos = envs.step(act)
                else:
                    a = action.cpu().numpy().reshape(envs.num_envs, -1)
                    next_obs, rs, ds, infos = envs.step(a)
                org_next_obs = next_obs
